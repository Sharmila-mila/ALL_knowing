    ...
    for term in terms:
        ...
        if fh.ok and isinstance(fh.data, dict):
            ...
            if search.get("result"):
                ...
                if ticker and (term.upper() == query.upper() or _name_matches_term(name, term) or any(_name_matches_term(name, a) for a in aliases)):
...
async def resolve_identity(http: HttpClient, limits: RateLimits, query: str) -> CompanyContext:
    ...
    # Keep the user's query as the identity anchor. Search providers can return
    # similarly named organizations; they must not silently rename the dossier.
    ctx.name = query.strip()
    ...
    if wiki.ok and isinstance(wiki.data, dict) and not wiki_vague and not wiki_generic:
        ...
        if wiki_name and ctx.ticker and not _plausible_ticker(ctx.ticker, wiki_name):
    ...
    print(f"Input Company: {query}", flush=True)
    print(f"Resolved Identity: {ctx.name}", flush=True)
    print(f"Resolved Ticker: {ctx.ticker}", flush=True)
    return ctx
