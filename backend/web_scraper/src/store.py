...
SUPABASE_AVAILABLE = property(lambda: using_db())


def __getattr__(name: str) -> Any:
    if name == "SUPABASE_AVAILABLE":
        return using_db()
...
def _canonical_company_linkedin(value: Any) -> str | None:
    ...
    if not re.fullmatch(
        ...
        return None
    match = re.search(r"/company/([^/?#]+)$", raw, re.I)
    return f"https://www.linkedin.com/company/{match.group(1)}/" if match else None


def _extract_company_fields(dossier: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(dossier, dict):
        return {
            "website": None,
            "linkedin_url": None,
    ...
    resolved = dossier.get("resolved") if isinstance(dossier.get("resolved"), dict) else {}
    overview = dossier.get("overview") if isinstance(dossier.get("overview"), dict) else {}
    li = dossier.get("linkedin_insights") if isinstance(dossier.get("linkedin_insights"), dict) else (dossier.get("linkedin") if isinstance(dossier.get("linkedin"), dict) else {})
    financials = dossier.get("financials") if isinstance(dossier.get("financials"), dict) else {}

    def _val(*vals: Any) -> Any | None:
        for v in vals:
            if v is not None:
                s = str(v).strip()
    ...
    return {
        "website": _val(dossier.get("website"), resolved.get("website"), overview.get("website"), li.get("website")),
        "linkedin_url": _canonical_company_linkedin(
            _val(dossier.get("linkedin_url"), li.get("linkedin_url"), overview.get("linkedin_url"))
        ),
        "company_logo": _val(dossier.get("company_logo"), li.get("company_logo"), overview.get("company_logo"), overview.get("logo_url"), overview.get("thumbnail_url")),
        "industry": _val(dossier.get("industry"), li.get("industry"), overview.get("industry"), overview.get("sector")),
        "company_type": _val(dossier.get("company_type"), li.get("company_type"), overview.get("company_type")),
        "founded_year": _to_int(_val(dossier.get("founded_year"), li.get("founded_year"), overview.get("founded_year"), overview.get("founded"))),
        "employee_count": _to_int(_val(dossier.get("employee_count"), li.get("employee_count"), overview.get("employee_count"), overview.get("employees"))),
        "linkedin_followers": _to_int(_val(dossier.get("linkedin_followers"), li.get("linkedin_followers"), overview.get("linkedin_followers"))),
        "linkedin_members": _to_int(_val(dossier.get("linkedin_members"), li.get("linkedin_members"), overview.get("linkedin_members"))),
        "company_size": _val(dossier.get("company_size"), li.get("company_size"), overview.get("company_size")),
        "global_offices": _to_int(_val(dossier.get("global_offices"), li.get("global_offices"), overview.get("global_offices"))),
        "hiring_status": _val(dossier.get("hiring_status"), li.get("hiring_status"), overview.get("hiring_status")),
        "market_cap": _to_float(_val(financials.get("market_cap"))),
        "annual_revenue": _to_float(_val(financials.get("annual_revenue"), financials.get("revenue"), financials.get("revenue_ttm"))),
        "net_income": _to_float(_val(financials.get("net_income"))),
        "description": _val(overview.get("description"), li.get("description"), overview.get("short_description")),
    }


def _merge_preserving_existing(existing: Any, incoming: Any) -> Any:
    """Merge fresh data without allowing an empty scrape to erase saved data."""
    if isinstance(existing, dict) and isinstance(incoming, dict):
        merged = dict(existing)
        for key, value in incoming.items():
            if key in merged:
                merged[key] = _merge_preserving_existing(merged[key], value)
...
def _existing_company_row(key: str) -> dict[str, Any]:
    client = _sb()
    if client is not None:
        try:
            rows = client.table("companies").select("*").eq("key", key).limit(1).execute().data or []
            return rows[0] if rows and isinstance(rows[0], dict) else {}
        except Exception:
            return {}
    data = _read_json(COMPANY_DIR / f"{key}.json")
    return data if isinstance(data, dict) else {}


def _merge_company_dossier(key: str, dossier: dict[str, Any]) -> dict[str, Any]:
    existing_row = _existing_company_row(key)
    existing_dossier = existing_row.get("dossier") if isinstance(existing_row.get("dossier"), dict) else existing_row
    existing_dossier = dict(existing_dossier or {})
    overview = dict(existing_dossier.get("overview") or {})
    column_overview = {
        "website": existing_row.get("website"),
        "linkedin_url": existing_row.get("linkedin_url"),
        "linkedin_followers": existing_row.get("linkedin_followers"),
        "linkedin_members": existing_row.get("linkedin_members"),
        "employee_count": existing_row.get("employee_count"),
        "description": existing_row.get("description"),
        "headquarters": existing_row.get("headquarters"),
        "industry": existing_row.get("industry"),
    }
    for field, value in column_overview.items():
        if value not in (None, "", []):
            overview.setdefault(field, value)
    if overview:
        existing_dossier["overview"] = overview
    return _merge_preserving_existing(existing_dossier, dossier)



def _persist_company(key: str, dossier: dict[str, Any], updated_at: float | None = None, disk: bool = False) -> None:
    dossier = _merge_company_dossier(key, dossier)
    ...
    extracted = _extract_company_fields(dossier)
    record = {
        "key": key,
        "query": dossier.get("query") or key,
        "ticker": resolved.get("ticker") or None,
        "name": resolved.get("name") or (dossier.get("overview") or {}).get("legal_name") or None,
        "dossier": _jsonable(dossier),
        "updated_at": stamp,
        **extracted,
    }
    payload_log = {k: v for k, v in record.items() if k != "dossier"}
    print('dossier["linkedin_url"] -> ' + str(dossier.get("linkedin_url")), flush=True)
    print(
        "Supabase Payload: "
        + json.dumps(
            {
                ...
                "linkedin_followers": record.get("linkedin_followers"),
                "linkedin_members": record.get("linkedin_members"),
                "employee_count": record.get("employee_count"),
                "description": record.get("description"),
                "headquarters": record.get("global_offices"),
    ...
    print("[Supabase Store] Payload before upsert:", flush=True)
    print(json.dumps(payload_log, indent=2, default=str), flush=True)
    try:
        client.table("companies").upsert(record).execute()
        print("UPSERT SUCCESS", flush=True)
        print("\n[Supabase Store] Final payload written to Supabase:")
        print(json.dumps(payload_log, indent=2, default=str))
    except Exception as exc:
        print(f"[Supabase Store Error] {exc}")
...
def upsert_person(record: dict[str, Any]) -> dict[str, Any]:
    linkedin_value = str(
        ...
        or record.get("linkedin_profile_url")
        or record.get("url")
        or ""
    ).strip().split("?")[0].rstrip("/")
    linkedin_match = re.fullmatch(
        r"https?://(?:www\.)?linkedin\.com/in/([^/#?]+)",
        linkedin_value,
        re.I,
    )
    if linkedin_value and not linkedin_match:
        record = dict(record)
        record.pop("linkedin_url", None)
        record.pop("linkedin_profile_url", None)
        record.pop("url", None)
        linkedin_value = ""
    elif linkedin_match:
        linkedin_value = f"https://www.linkedin.com/in/{linkedin_match.group(1)}"
        record = dict(record)
        record["linkedin_url"] = linkedin_value
    ...
    payload = {
        ...
        "linkedin_url": linkedin_value or (str(prev.get("linkedin_url") or "").strip() if "/in/" in str(prev.get("linkedin_url") or "") else None),
