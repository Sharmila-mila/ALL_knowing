...
def _fetch_voyager_json(page: Page, path: str) -> Any | None:
    ...
    # Some LinkedIn pages expose the session cookie to the browser but reject
    # fetch() requests made from the page. Reuse the authenticated context as
    # a second path so profile data is still available when that happens.
    try:
        cookies = page.context.cookies("https://www.linkedin.com")
        jsession = next(
            (str(item.get("value") or "") for item in cookies if item.get("name") == "JSESSIONID"),
            "",
        ).strip('"')
        if not jsession:
            return None
...
def extract_profile(page: Page, url: str) -> dict[str, Any]:
    target_url = _normalize_profile_url(url) or url
    data = _blank_result(target_url)
    captured: list[dict[str, Any]] = []
    handler = _attach_response_capture(page, captured)
    slug = _slug_from_url(url)

    try:
        page.goto(target_url, wait_until="domcontentloaded", timeout=45000 if _fast() else 60000)
        ...
        try:
            page.wait_for_selector(
                "main h1, h1.text-heading-xlarge, h1.break-words, .pv-text-details__left-panel h1",
                timeout=10000 if _fast() else 16000,
            )
        except Exception:
            pass
