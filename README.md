...
```text
.
├── frontend/               # Web UI SPA static assets and screenshots
│   ├── front/              # index.html, css/, js/
│   └── assets/             # README screenshots & branding
├── backend/                # Server & Intelligence pipelines
│   ├── ui/                 # Flask app and API routes (app.py)
│   ├── web_scraper/        # Company market intelligence pipeline
│   ├── lead_finder/        # Email → company orchestration & discovery
│   └── lead_scraper/       # Playwright LinkedIn profile extraction & enrichment
├── integration/            # Integrations & Database Schemas
│   ├── extension/          # Chrome MV3 side-panel extension
│   └── supabase/           # schema.sql database definitions
├── run_production.py       # Multi-threaded production server launcher (Waitress/Gunicorn)
├── wsgi.py                 # Standard WSGI entrypoint
├── Dockerfile              # Docker container deployment spec
├── docker-compose.yml      # 1-command Docker environment orchestration
